...
const checkAccess = (user, password) => {
    return user === "admin" && password === 123456 ? "Доступ разрешен" : "Доступ запрещен";
}
...
const getTimeOfDay = time => time >= 0 && time <= 5 ? "Night" : (
    time >= 6 && time <= 11 ? "Morning" : (
        time >= 12 && time <= 17 ? "Day" : (
            time >= 18 && time <= 23 ? "Evening" : "Incorrect time")
    )
);
